-- Fix block_hospital_self_approval to allow hospitals to update to necessary statuses
...
BEGIN
  ...
  IF public.has_role(auth.uid(), 'hospital') THEN
    ...
    -- ALLOW hospitals to accept/decline referrals, submit treatments, and provide info
    IF NEW.status IS DISTINCT FROM OLD.status THEN
      IF NEW.status NOT IN ('referral_accepted', 'referral_declined', 'pending_authorization', 'info_provided', 'pending') THEN
