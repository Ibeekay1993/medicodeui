...
// We rely on the configured X-Webhook-Token header for webhook authentication.
//
// Security model:
//   - Every inbound message is retained for audit/reliability.
//   - Only an ACTIVE `hospital` user in `user_roles` with a matching phone and
//     hospital_id is allowed to enter the AI/authorization worker.
...
async function resolveHospitalSender(
  ...
  if (error) {
    console.error("evolution-webhook: hospital identity lookup failed", error.message);
    return { authorized: false as const, reason: "identity_lookup_failed" };
  }

  const matches = (data || []).filter(
    (row: any) => normalizePhoneNumber(String(row.phone || "")) === normalized,
  );
  const hospitals = [
    ...new Set(matches.map((row: any) => String(row.hospital_id)).filter(Boolean)),
  ];
  ...
  const contact = matches.find(
    (row: any) => String(row.hospital_id) === hospitals[0],
  );
...
async function sendRestrictedReply(toPhone: string) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "evolution-webhook: restricted reply failed",
        response.status,
        body.slice(0, 200),
      );
    }
    ...
    console.error(
      "evolution-webhook: restricted reply threw",
...
function extractMessageContent(
  ...
  const ext = (message as Record<string, Record<string, unknown>>)
    .extendedTextMessage;
...
serve(async (req) => {
  ...
  for (let i = 0; i < min; i++) {
    diff |= provided.charCodeAt(i) ^ EVOLUTION_WEBHOOK_TOKEN.charCodeAt(i);
  }
  ...
  if (instance && instance !== EVOLUTION_INSTANCE_NAME) {
    return jsonResponse({
      ok: true,
      ignored: "instance_mismatch",
      expected: EVOLUTION_INSTANCE_NAME,
      got: instance,
    });
  }
  ...
  let insErr: { message: string } | null = null;
  {
    const first = await supabase
      ...
      .insert({ ...insertRow, media_url: mediaUrl });
  ...
  if (supabaseUrl && serviceKey) {
    try {
      ...
      fetch(`${supabaseUrl}/functions/v1/whatsapp-worker`, {
        ...
        console.error("worker invoke failed", e?.message || String(e)),
      );
    } catch (e) {
