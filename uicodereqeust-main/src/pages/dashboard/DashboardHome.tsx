...
export default function DashboardHome() {
  ...
  return (
    <div className="space-y-4 max-w-full overflow-x-hidden pb-10 animate-in fade-in duration-500">
                ...
                <div key={item.label} className="premium-card flex flex-col justify-center items-center sm:items-start sm:flex-row gap-1 sm:gap-3 p-1.5 sm:p-4 min-w-0 rounded-xl" title={item.label}>
                  ...
                  <div className="min-w-0 flex-1 text-center sm:text-left">
                    ...
                    {loading ? (
                      ...
                      <p className="mt-0.5 sm:mt-1.5 text-sm sm:text-lg font-extrabold tabular-nums leading-none text-slate-900 truncate">
      ...
      {isNurseOrOther && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-2.5 overflow-x-hidden">
          {mainStats.map((item) => {
            return (
              <div key={item.label} className="premium-card p-1.5 sm:p-4 flex flex-col justify-center min-w-0 text-center sm:text-left rounded-xl" title={item.label}>
                ...
                {loading ? (
                  ...
                  <p className="mt-0.5 sm:mt-1 text-sm sm:text-lg font-extrabold leading-none text-slate-900 truncate tabular-nums">
                    {Number(item.value).toLocaleString()}
                  </p>
                )}
              </div>
      ...
      {isClaims && (
        <div className="grid grid-cols-4 lg:grid-cols-7 gap-1 sm:gap-3">
          {[
