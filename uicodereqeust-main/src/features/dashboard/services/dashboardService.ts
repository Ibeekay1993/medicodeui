...
export class DashboardService {
  ...
  static async getAdminStats() {
    const [claims, approved, rejected, pending, hospitals, users, pendingCodes] = await Promise.all([
      ...
      supabase.from("hospital_claims" as any).select("*", { count: "estimated", head: true }).or("status.eq.rejected,status.eq.contested"),
      supabase.from("hospital_claims" as any).select("*", { count: "estimated", head: true }).eq("status", "pending"),
      supabase.from("hospitals").select("*", { count: "estimated", head: true }),
      supabase.from("user_roles").select("id", { count: "estimated", head: true }),
      supabase.from("historical_codes" as any).select("*", { count: "estimated", head: true }).eq("record_status", "pending"),
    ]);
  ...
  static async getClaimsOfficerStats() {
    const [total, approved, partialApproved, rejected, contested, hospitals, users, pendingCodes] = await Promise.all([
      ...
      supabase.from("authorization_requests").select("*", { count: "estimated", head: true }).eq("status", "partially_approved"),
