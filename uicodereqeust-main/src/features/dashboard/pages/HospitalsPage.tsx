...
export default function HospitalsPage() {
  ...
  const { data: hospitals = [], isLoading: loading, refetch: fetchHospitals } = useHospitalsPaged();
  const { data: hospitalUsers = [] } = useHospitalUsers();
  ...
  const handleUpdateHospital = async () => {
    ...
    await updateHospital({ id: editing.id, payload: updates });
    setEditing(null);
  };
  ...
  const executeDelete = async () => {
    ...
    await deleteHospitalAction(deleteTarget.id);
  ...
  const linkHospitalUser = async (userId: string | null) => {
    ...
    await linkUser({ userId: userId || "", hospitalId: linking.id });
    setLinking(null);
  };
